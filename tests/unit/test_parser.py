...
for network_os in list(compliance.parser_map.keys()):
    ...
    for _file in glob.glob(f"{MOCK_DIR}/find_children/{network_os}/*{TXT_FILE}"):
        find_children_parameters.append([_file, network_os])
...
@pytest.mark.parametrize("_file, network_os", find_children_parameters)
def test_find_children(_file, network_os, get_text_data, get_json_data):  # pylint: disable=redefined-outer-name
    truncate_file = os.path.join(MOCK_DIR, "find_children", _file[: -len(TXT_FILE)])

    device_cfg = get_text_data(os.path.join(MOCK_DIR, "find_children", _file))
    received_data = get_text_data(os.path.join(MOCK_DIR, "find_children", truncate_file + "_received.txt"))
    kwargs = get_json_data(truncate_file + "_args.json")
    os_parser = compliance.parser_map[network_os]
    assert "\n".join(os_parser(device_cfg).find_children(**kwargs)) == received_data
